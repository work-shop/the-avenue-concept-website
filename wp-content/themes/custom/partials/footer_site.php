<footer class="block pt6 bg-dark" id="footer">
	<div class="container-fluid">
		<div class="row mb3">
			<div class="col-md-6 col-lg-3 footer-col">
				<div id="logo-footer" class="logo">
					<a href="/" title="The Avenue Concept">
						<img src="<?php bloginfo( 'template_directory' ); ?>/images/logo-white.svg" title="The Avenue Concept Logo" alt="The Avenue Concept Logo">
					</a>
				</div>
			</div>
		</div>
		<div class="row mb6">
			<div class="col-md-6 col-lg-3 footer-col">
				<address class="footer-address">
					The Avenue Concept<br>
					304 Lockwood Avenue<br>
					Providence, RI 02907
				</address>
			</div>
			<div class="col-md-6 col-lg-3 footer-col">
				<p>
					Contact Us:<br>
					<a href="mailto:hello@theavenueconcept.com" class="white" target="_blank">hello@theavenueconcept.com</a><br>
					(401) 490-0929
				</p>
			</div>
			<div class="col-md-6 col-lg-3 col-12 footer-col">
				<p>
					<?php the_field('footer_landing_page_message'); ?>
				</p>
			</div>	
			<div class="col-md-6 col-lg-3 col-12 footer-col d-flex justify-content-center">
				<a href="https://www.facebook.com/TheAvenueConcept" target="_blank">
					<img src="<?php bloginfo( 'template_directory' );?>/images/facebook.png" class="social-icon">
				</a> 											
				<a href="https://www.instagram.com/avenuepvd" target="_blank">
					<img src="<?php bloginfo( 'template_directory' );?>/images/instagram.png" class="social-icon">
				</a> 
				<a href="https://twitter.com/avenuepvd?lang=en" target="_blank">
					<img src="<?php bloginfo( 'template_directory' );?>/images/twitter.png" class="social-icon">
				</a>
			</div>				
		</div>
		<div class="site-credit pb1">
			<a href="http://workshop.co" target="_blank">
				<h5 class="white font-primary mb0">Site by Work-Shop Design Studio</h5>
			</a>
		</div>
	</div>
</footer>